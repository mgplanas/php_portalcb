...
if(isset($_POST['action']) or isset($_GET['view'])) //show all events
{
    ...
    {   
        mysqli_query($con,"INSERT INTO activacion_guardia (titulo , start , end )
                    VALUES (
    ...
    {
        $id_activacion_guardia = $_POST["id"];
        $persona_guardia = mysqli_query($con, "SELECT persona FROM activacion_guardia WHERE id_activacion_guardia='$id_activacion_guardia'");
        $rowpac = mysqli_fetch_assoc($persona_guardia);
        $id_rowpac = $rowpac['persona'];
        
        if ( $id_rowpac == $id_rowp){
            ...
            $update_activacion_guardia = mysqli_query($con, "UPDATE activacion_guardia SET 
                startDay = '$startDay', startTime = '$startTime', endDay = '$endDay', endTime ='$endTime'
                WHERE id_activacion_guardia = '$id_activacion_guardia'") or die(mysqli_error());
            
            $lastInsert = mysqli_insert_id($con);
            $insert_audit = mysqli_query($con, "INSERT INTO auditoria (evento, item, id_item, fecha, usuario) 
                                                VALUES ('2', '13', '$lastInsert', now(), '$user')") or die(mysqli_error());
             exit;
    }
    }
    ...
    {
        mysqli_query($con,"DELETE from activacion_guardia where id_activacion_guardia = '".mysqli_real_escape_string($con,$_POST["id"])."'");
  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        echo '<li class="active"><a href="calendario.php"><i class="fa fa-calendar"></i> <span>Calendario</span></a></li>';
    ...
    <section class="content">
      <div class="row">
        ...
        echo '<div class="form-group">
                <button type="button" class="btn-block btn-primary" data-toggle="modal" data-target="#modal-guardia"> Cargar activación de guardia pasiva</button>
                
         </div>';
        }
        ?>   
         <div class="form-group">
             <input type="button" class="btn-block btn-primary" onclick="location.href='./calendario.php';" value="Volver al calendario"/>
         </div>
        Aclaración: la liquidación de guardias corresponde al rango mensual entre el 10 y el 11. Siendo liquidadas a mes vencido.
        </div>
        <!-- /.col -->
        <div class="col-md-9">
...
<!-- custom scripts --> 
<script type="text/javascript" src="../js/script_guardias.js"></script> 
