...
$current_version = 0;
$last_version = 0;
$sqllastver = mysqli_query($con, "SELECT id FROM bcra_version WHERE borrado=0 ORDER BY modificacion desc LIMIT 1");
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <select id="versionselector" name="responsable" class="form-control">
            <?php
              $versiones = mysqli_query($con, "SELECT * FROM bcra_version WHERE borrado = 0 ORDER BY modificacion desc ");
        ...
        <div class="col-sm-8" style="text-align:right;">
          ...
          if ($current_version == $last_version) {
            echo '<button type="button" id="modal-abm-bcra-btn-alta" class="btn-sm btn-primary" data-toggle="modal" data-target="#modal-activo"><i class="fa fa-refresh"></i> Nuevo ítem</button>';
          }
          ?>
        </div>        
            </div>
        <!-- /.modal Activo-->
            <!-- /.box-header -->		
			<div class="box-body">
              <table id="bcra" class="display" width="100%">
                <thead>
                <tr>
                  <th width="1"></th>
                  <th width="1"></th>
		  <th width="2">Codigo</th>
                ...
                <tbody>
                  ...
                  (
                    ...
                    WHERE r.id_item_bcra = i.id_item_bcra
                    GROUP BY r.id_item_bcra
                  ) as referentes,
                  (
                  ...
                  tit.id_item_bcra as t_id, tit.codigo as t_codigo, tit.titulo as t_titulo, tit.descripcion as t_descripcion
                  FROM item_bcra as i 
                  LEFT JOIN madurez as m on i.madurez = m.id_madurez 
                  LEFT JOIN persona as p on i.responsable = p.id_persona
                  LEFT JOIN item_bcra as tit on i.parent = tit.id_item_bcra
                  WHERE i.borrado='0'
                    AND i.nivel = 2 
                    AND i.version = " . $current_version;
                  
                  $sql = mysqli_query($con, $query.' ORDER BY tit.id_item_bcra, i.id_item_bcra ASC');
                  ...
                  while($row = mysqli_fetch_assoc($sql)){
                    ...
                    // href="edit_bcra.php?nik='.$row['id_item_bcra'].'&version='. $current_version .'"
                    if ($current_version == $last_version) {
                      echo '
                      <td align="center">
                        <a 
                          data-id="'.$row['id_item_bcra'].'"
                        ...
                        // echo '<a data-id="'.$row['id_item_bcra'].'" data-codigo="'.$row['codigo'].'" title="Borrar datos" class="btn btn-danger btn-sm modal-abm-bcra-btn-baja';
                        // if ($rq_sec['edicion']=='0'){
                        //   echo 'disabled';
                        // }
        ...
        <!-- /.col -->
        <?php
            include_once('./modals/abmbcra.php');
        ?>        
      </div>
      <!-- /.row -->
    </section>
...
<script src="./modals/abmbcra.js"></script>  
...
<script>
  $(function () {
    ...
    $('#versionselector').on('change', function() {
      window.location.href = "bcra.php?version=".concat(this.value);
    });    
