          ...
          <div class="box">
			...
			<div class="box-body">
              <table id="entes" class="table table-bordered table-hover">
                ...
                <tbody>
					<?php
					$query = "SELECT O.id, O.razon_social, O.cuit, O.observaciones, (SELECT COUNT(*) FROM aud_auditores AS C WHERE C.id_ente = O.id) AS auditores
                    FROM aud_entes AS O
                    WHERE O.borrado = 0 
					...
					if(mysqli_num_rows($sql) == 0){
						...
						while($row = mysqli_fetch_assoc($sql)){
                            ...
                            if ($rq_sec['admin']=='1' OR $rq_sec['edicion']=='1'){
                                echo '<a href="aud_auditor.php?nik='.$row['id'].'" title="Auditores" class="btn btn-sm"><i class="fa fa-users"></i></a>';
                                echo '<a 
                                    data-id="' . $row['id'] . '" 
